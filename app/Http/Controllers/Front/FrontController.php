...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        return view('site.home', compact('postCategories','postsRecent','categoriesSpecial', 'about', 'achievements',
            'feedbacks', 'tags', 'banners', 'services','partners', 'categoriesSpecial', 'postCategories', 'postsRecent'));
    }
    ...
    public function getPostByCate(Request $request) {
        ...
        if(! $category->parent_id) {
            ...
            $cateIds = array_merge($lv2Ids->merge($lv3Ids)->unique()->values()->all(), [$category->id]);
        }
    ...
    public function getPostByTag(Request $request, $slug) {
        ...
        $posts = Post::query()
            ...
            ->paginate(9, ['posts.*'])->appends($request->only('sort'));
        ...
        // bài viết mới nhất
        $postsRecent = Post::query()->with(['category', 'image'])->where('status', 1)->latest()->limit(6)->get();
        $banner = Banner::query()->with('image')->where('type',8)->first();

        return view('site.blog_tag', compact('posts', 'tag', 'categories', 'tags', 'postsRecent', 'banner'));
    }
    ...
    public function blogs(Request $request, $slug = null) {
        ...
        if($slug) {
            ...
            if(! $category->parent_id) {
                ...
                if($category->level == 1) {
                    ...
                    $cateIds = array_merge([$category->id], $lv3Ids);
        ...
        return view('site.blog', compact('posts', 'category', 'categories', 'tags', 'postsRecent', 'parentCateLv1', 'parentCateLv2', 'banner'));
    }
    ...
    public function blogDetail(Request $request, $slug) {
        ...
        return view('site.blog_detail', compact('blog', 'othersBlog', 'categories', 'tags', 'banner'));
    }
    ...
    public function searchProduct(Request $request) {
        ...
        return view('site.search', compact('posts', 'keyword', 'categories', 'tags', 'banner'));
    }
    ...
    public function policy(Request $request, $slug) {
        ...
        $banner = Banner::query()->with('image')->where('type', 11)->first();

        return view('site.chinh_sach', compact('policy', 'banner'));
    }
